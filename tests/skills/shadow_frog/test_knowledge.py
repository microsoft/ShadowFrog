r"""Tests for shared `skills/shadow-frog/_knowledge.py` and the user viewer CLI.
...
Test categories:
  * In-process function tests (no `@pytest.mark.slow`): exercise
    parsing helpers directly via the `shadow_knowledge` fixture.
...
def test_parse_discovery_with_labels(shadow_knowledge):
...
def test_parse_discovery_status_variants(shadow_knowledge, status):
    line = "- Some discovery"
...
def test_parse_discovery_source_variants(shadow_knowledge, source):
    line = "- Some discovery"
...
def test_parse_discovery_also_involves(shadow_knowledge):
...
def test_parse_discovery_b3_dream_report_regression(shadow_knowledge):
...
def test_parse_discovery_b3_dream_report_with_also_involves(shadow_knowledge):
...
def test_parse_discovery_multiline_body(shadow_knowledge):
...
def test_parse_discovery_no_metadata(shadow_knowledge):
    """Bullet with no metadata blob still returns a dict with text but no
    status/source keys."""
    d = shadow_knowledge.parse_discovery("- bare bullet", [])
    assert d["text"] == "bare bullet"
    assert "status" not in d
    assert "source" not in d


def test_parse_discovery_preferences_source_only(shadow_knowledge):
    """Preferences use `_(source: user)_` (no status). Extracts source."""
    d = shadow_knowledge.parse_discovery(
...
def test_parse_discovery_none_input_does_not_crash(shadow_knowledge):
    """Passing a non-string line shouldn't raise — should return a dict."""
    d = shadow_knowledge.parse_discovery(None, None)
...
def test_parse_shadow_file_placeholder(shadow_knowledge, tmp_path):
    ...
    """)
    res = shadow_knowledge.parse_shadow_file(f)
...
def test_parse_shadow_file_one_symbol_one_discovery(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
...
def test_parse_shadow_file_file_level_and_cross_refs(shadow_knowledge, tmp_path):
...
def test_parse_shadow_file_malformed_does_not_crash(shadow_knowledge, tmp_path):
...
def test_parse_shadow_file_missing_file_returns_error(
    ...
    res = shadow_knowledge.parse_shadow_file(tmp_path / "ghost.md")
    assert res["parse_errors"]
    assert res["symbols"] == []
    assert res["discoveries"] == []
...
def test_parse_cross_cutting_empty_dir(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    (sd / "_cross").mkdir()
    assert shadow_knowledge.parse_cross_cutting(sd) == []


def test_parse_cross_cutting_no_dir(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    # _cross/ not created
    assert shadow_knowledge.parse_cross_cutting(sd) == []


def test_parse_cross_cutting_one_entry(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    ...
    """)
    entries = shadow_knowledge.parse_cross_cutting(sd)
...
def test_parse_cross_cutting_no_labels(shadow_knowledge, tmp_path):
...
def test_parse_cross_cutting_minor_format_variation(shadow_knowledge, tmp_path):
...
def test_parse_prefs_missing_file(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    assert shadow_knowledge.parse_prefs(sd) == []


def test_parse_prefs_zero(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
...
def test_parse_prefs_one(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    _write_shadow(sd, "_prefs.md", """\
        # Preferences
        ...
        - Always use type hints on public APIs.
          _(source: user)_
    """)
    prefs = shadow_knowledge.parse_prefs(sd)
    assert len(prefs) == 1
...
def test_parse_prefs_three(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    _write_shadow(sd, "_prefs.md", """\
        # Preferences
...
def test_load_state_missing(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    assert shadow_knowledge.load_state(sd) == {}


def test_load_state_valid_json(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    (sd / "_meta").mkdir()
    payload = {
        "version": 1,
    ...
    state = shadow_knowledge.load_state(sd)
    assert state == payload


def test_load_state_malformed_json(shadow_knowledge, tmp_path):
    sd = _make_shadow_root(tmp_path)
    (sd / "_meta").mkdir()
    (sd / "_meta" / "state.json").write_text(
...
def test_load_state_non_object_json(shadow_knowledge, tmp_path):
...
def test_get_all_shadow_files_excludes_special(shadow_knowledge, tmp_path):
    ...
    files = shadow_knowledge.get_all_shadow_files(sd)
    rels = sorted(f.relative_to(sd).as_posix() for f in files)
    assert rels == ["a.py.md", "deep/nested/c.py.md", "src/b.py.md"]


def test_get_all_shadow_files_against_coupon_demo(shadow_knowledge, coupon_demo):
...
def test_collect_all_discoveries_counts(shadow_knowledge, coupon_demo):
    """Coupon demo has 33 per-file discoveries (matches state.json)."""
    sd = coupon_demo / ".shadow"
    all_disc = shadow_knowledge.collect_all_discoveries(sd)
...
# `shadow_knowledge` fixture so coverage actually accumulates.
# ===========================================================================
...
def _call_main(shadow_knowledge, argv):
    """Invoke `shadow_knowledge.main()` in-process with the given argv.
...
class TestViewSummary:
    ...
    def test_basic_header_on_coupon_demo(
        self, shadow_knowledge, coupon_demo, capsys
    ):
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_summary(sd)
    ...
    def test_empty_shadow_dir_renders_zeros(
        self, shadow_knowledge, tmp_path, capsys
    ):
        sd = _make_shadow_root(tmp_path)
        shadow_knowledge.view_summary(sd)
        out = capsys.readouterr().out
    ...
    def test_summary_no_prefs(self, shadow_knowledge, tmp_path, capsys):
        sd = _make_minimal_shadow(tmp_path)
        shadow_knowledge.view_summary(sd)
        out = capsys.readouterr().out
        assert "Preferences:       0" in out

    def test_summary_with_prefs(self, shadow_knowledge, tmp_path, capsys):
        sd = _make_minimal_shadow(tmp_path)
        _write_shadow(sd, "_prefs.md", """\
            # Preferences
...
class TestViewSearch:
    ...
    def test_finds_text_match(self, shadow_knowledge, coupon_demo, capsys):
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_search(sd, "tax")
        out = capsys.readouterr().out
    ...
    def test_finds_symbol_match(self, shadow_knowledge, coupon_demo, capsys):
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_search(sd, "COUPON_CACHE")
        out = capsys.readouterr().out
    ...
    def test_finds_file_name_match(
        ...
        shadow_knowledge.view_search(sd, "inventory.py")
        out = capsys.readouterr().out
        assert "inventory.py" in out
        assert "matches" in out

    def test_case_insensitive(self, shadow_knowledge, coupon_demo, capsys):
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_search(sd, "COUPON")
    ...
    def test_no_matches_prints_no_results(
        ...
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_search(sd, "zzzdoesnotexistzzz")
        out = capsys.readouterr().out
    ...
    def test_finds_cross_cutting_title(
        ...
        shadow_knowledge.view_search(sd, "normalization mismatch")
        out = capsys.readouterr().out
        assert "Cross-cutting" in out
        assert "Coupon case normalization mismatch" in out
    ...
    def test_finds_cross_cutting_by_ref(
        ...
        shadow_knowledge.view_search(sd, "apply_bulk_discount")
        out = capsys.readouterr().out
        assert "Cross-cutting" in out
    ...
    def test_finds_also_involves_match(
        ...
        """)
        shadow_knowledge.view_search(sd, "weird_symbol_zzz")
        out = capsys.readouterr().out
    ...
    def test_finds_preference_match(
        ...
        _write_shadow(sd, "_prefs.md", """\
            ...
            - Prefer rusty pelicans for everything.
              _(source: user)_
        """)
        shadow_knowledge.view_search(sd, "pelican")
        out = capsys.readouterr().out
        assert "Preferences" in out
    ...
    def test_empty_shadow_returns_no_results(
        ...
        sd = _make_shadow_root(tmp_path)
        shadow_knowledge.view_search(sd, "anything")
        out = capsys.readouterr().out
...
class TestViewPrefs:
    ...
    def test_missing_prefs_file(self, shadow_knowledge, tmp_path, capsys):
        sd = _make_shadow_root(tmp_path)
        shadow_knowledge.view_prefs(sd)
        out = capsys.readouterr().out
...
class TestViewLabels:
    ...
    def test_each_label_returns_results_on_fixture(
        self, shadow_knowledge, coupon_demo, capsys, label
    ):
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_labels(sd, label)
        out = capsys.readouterr().out
    ...
    def test_unknown_label_no_results(
        ...
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_labels(sd, "zznotalabelzz")
        out = capsys.readouterr().out
    ...
    def test_multiple_labels_comma_split(
        ...
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_labels(sd, "bug,security")
        out = capsys.readouterr().out
    ...
    def test_label_filter_is_lowercased(
        ...
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_labels(sd, "BUG")
        out = capsys.readouterr().out
    ...
    def test_cross_cutting_labels_included(
        ...
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_labels(
            sd, "bug", options=shadow_knowledge.RetrievalOptions(limit=20, max_chars=8000),
        )
        out = capsys.readouterr().out
    ...
    def test_each_result_row_has_file_and_symbol(
        ...
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_labels(sd, "feature-gap")
        out = capsys.readouterr().out
...
class TestViewRecent:
    ...
    def test_default_count_caps_at_10(
        ...
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_recent(sd, 10)
        out = capsys.readouterr().out
    ...
    def test_custom_count(self, shadow_knowledge, coupon_demo, capsys):
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_recent(sd, 3)
        out = capsys.readouterr().out
    ...
    def test_count_larger_than_available(
        ...
        sd = _make_minimal_shadow(tmp_path)
        shadow_knowledge.view_recent(sd, 50)
        out = capsys.readouterr().out
    ...
    def test_includes_cross_cutting_type(
        ...
        shadow_knowledge.view_recent(sd, 5)
        out = capsys.readouterr().out
        assert "(cross-cutting)" in out
    ...
    def test_entries_include_timestamp_and_kind(
        ...
        sd = coupon_demo / ".shadow"
        shadow_knowledge.view_recent(sd, 2)
        out = capsys.readouterr().out
...
class TestViewCheckInvariants:
    ...
    def test_clean_coupon_demo_returns_zero(
        ...
        rc = shadow_knowledge.view_check_invariants(coupon_demo / ".shadow")
        out = capsys.readouterr().out
        assert rc == 0
    ...
    def test_empty_shadow_returns_zero(
        ...
        sd = _make_shadow_root(tmp_path)
        rc = shadow_knowledge.view_check_invariants(sd)
        out = capsys.readouterr().out
        assert rc == 0
        assert "Invariants OK" in out
    ...
    def test_invalid_status_enum(self, shadow_knowledge, tmp_path, capsys):
        sd = _make_shadow_root(tmp_path)
        _write_shadow(sd, "a.py.md", """\
            # Shadow: a.py
    ...
    def test_invalid_source_enum(self, shadow_knowledge, tmp_path, capsys):
        sd = _make_shadow_root(tmp_path)
        _write_shadow(sd, "a.py.md", """\
            # Shadow: a.py
    ...
    def test_invalid_label(self, shadow_knowledge, tmp_path, capsys):
        sd = _make_shadow_root(tmp_path)
        _write_shadow(sd, "a.py.md", """\
            # Shadow: a.py
...
class TestMainInProcess:
    ...
    def test_summary_dispatch(
        ...
        rc = _call_main(
            shadow_knowledge, ["--shadow-dir", self._shadow(coupon_demo),
                            "--summary"]
        )
        out = capsys.readouterr().out
        assert rc == 0
    ...
    def test_explicit_shadow_dir_missing(
        ...
        rc = _call_main(
            shadow_knowledge, ["--shadow-dir", str(bogus), "--summary"]
        )
        err = capsys.readouterr().err
        assert rc == 1
    ...
    def test_auto_detect_via_chdir(
        self, shadow_knowledge, coupon_demo, monkeypatch, capsys
    ):
        """No --shadow-dir: cwd is walked up to find .shadow/."""
        monkeypatch.chdir(coupon_demo)
        rc = _call_main(shadow_knowledge, ["--summary"])
        out = capsys.readouterr().out
        assert rc == 0
        assert "Shadow Knowledge Base Summary" in out

    def test_auto_detect_no_shadow_in_cwd(
        self, shadow_knowledge, tmp_path, monkeypatch, capsys
    ):
